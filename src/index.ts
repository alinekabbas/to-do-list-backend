...
app.delete("/users/:id", async (req: Request, res: Response) => {
    try {
        ...
        if(idToDelete[0] !== "f"){
            res.status(400)
            throw new Error("'id' deve iniciar com letra 'f'");   
        }
        ...
        await db("users_tasks").del().where({user_id: idToDelete}) //deletar primeiro as relações
        await db("users").del().where({id: idToDelete}) //para depois deletar o usuário
    ...
    try {
        ...
        if(newDescription !== undefined){
            if(typeof newDescription !== "string"){
                res.status(400)
                throw new Error("'descrption' deve ser string");
            }
        }
        ...
        if(newCreatedAt !== undefined){
            if(typeof newCreatedAt !== "string"){
                res.status(400)
                throw new Error("'createdAd' deve ser string");
            }
        }
...
app.post("/tasks/:taskId/users/:userId", async (req: Request, res: Response) => {
    try {
        ...
        if(taskId[0] !== "t"){
            res.status(400)
            throw new Error("'taskId' deve iniciar com letra 't'");   
        }

        if(userId[0] !== "f"){
            res.status(400)
            throw new Error("'userId' deve iniciar com letra 'f'");   
        }
